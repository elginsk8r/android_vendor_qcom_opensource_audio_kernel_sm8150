			...
			for (j = 0; j < MAX_COPPS_PER_PORT; j++) {
				...
				if (test_bit(j, &copp)) {
					payload.port_id[num_copps] =
							get_port_id(msm_bedais[i].port_id);
					  ...
					  uint32_t passthr_mode)
{
	...
	for (i = 0; i < MSM_BACKEND_DAI_MAX; i++) {
		if (!is_be_dai_extproc(i) &&
			...
			adm_set_session_type(get_port_id(msm_bedais[i].port_id), session_type);

			copp_idx =
				adm_open(get_port_id(msm_bedais[i].port_id),
			...
			if (msm_is_resample_needed(
				...
				adm_copp_mfc_cfg(
					get_port_id(msm_bedais[i].port_id), copp_idx,
					msm_bedais[i].sample_rate);
	...
	for (i = 0; i < MSM_BACKEND_DAI_MAX; i++) {
		if (!is_be_dai_extproc(i) &&
		   ...
		   (test_bit(fedai_id, &msm_bedais[i].fe_sessions[0]))) {
			...
			copp_idx = adm_open(get_port_id(msm_bedais[i].port_id), path_type,
			...
			if (perf_mode == LEGACY_PCM_MODE)
				msm_pcm_routing_cfg_pp(get_port_id(msm_bedais[i].port_id),
		...
		if (!is_be_dai_extproc(i) &&
		   ...
		   (test_bit(fedai_id, &msm_bedais[i].fe_sessions[0]))) {
			...
			topology = adm_get_topology_for_port_copp_idx(
					get_port_id(msm_bedais[i].port_id), idx);
			msm_routing_unload_topology(topology);
			adm_close(get_port_id(msm_bedais[i].port_id), fdai->perf_mode, idx);
			...
			if ((topology == DOLBY_ADM_COPP_TOPOLOGY_ID ||
				...
				msm_pcm_routing_deinit_pp(get_port_id(msm_bedais[i].port_id),
							  topology);
		}
	}
...
static void msm_pcm_routing_process_audio(u16 reg, u16 val, int set)
{
	...
	if (set) {
		...
		if (msm_bedais[reg].active && fdai->strm_id !=
			...
			adm_set_session_type(get_port_id(msm_bedais[reg].port_id), session_type);
			copp_idx = adm_open(get_port_id(msm_bedais[reg].port_id), path_type,
					    sample_rate, channels, topology,
			...
			if (msm_is_resample_needed(
				...
				adm_copp_mfc_cfg(
					get_port_id(msm_bedais[reg].port_id), copp_idx,
					msm_bedais[reg].sample_rate);
			...
			if ((fdai->perf_mode == LEGACY_PCM_MODE) &&
				...
				msm_pcm_routing_cfg_pp(get_port_id(msm_bedais[reg].port_id),
						       copp_idx, topology,
						       channels);
		}
	} else {
		...
		if (msm_bedais[reg].active && fdai->strm_id !=
			...
			topology = adm_get_topology_for_port_copp_idx(port_id,
								      idx);
			msm_routing_unload_topology(topology);
			adm_close(get_port_id(msm_bedais[reg].port_id), fdai->perf_mode,
				  idx);
			...
			if ((topology == DOLBY_ADM_COPP_TOPOLOGY_ID ||
				...
				msm_pcm_routing_deinit_pp(
						get_port_id(msm_bedais[reg].port_id),
						topology);
			msm_pcm_routing_build_matrix(val, session_type,
						     path_type,
						     fdai->perf_mode,
...
static int msm_pcm_routing_close(struct snd_pcm_substream *substream)
{
	...
	for_each_set_bit(i, &bedai->fe_sessions[0], MSM_FRONTEND_DAI_MAX) {
		...
		if (fdai->strm_id != INVALID_SESSION) {
			...
			if ((fdai->perf_mode == LEGACY_PCM_MODE) &&
				(fdai->passthr_mode == LEGACY_PCM))
				msm_pcm_routing_deinit_pp(get_port_id(bedai->port_id),
							  topology);
		}
	}
...
static int msm_pcm_routing_prepare(struct snd_pcm_substream *substream)
{
	...
	for_each_set_bit(i, &bedai->fe_sessions[0], MSM_FRONTEND_DAI_MAX) {
		...
		if (fdai->strm_id != INVALID_SESSION) {
			...
			adm_set_session_type(get_port_id(bedai->port_id), session_type);
			copp_idx = adm_open(get_port_id(bedai->port_id), path_type,
					    sample_rate, channels, topology,
					    fdai->perf_mode, bits_per_sample,
					    app_type, acdb_dev_id);
			if ((copp_idx < 0) ||
			...
			if (msm_is_resample_needed(
				...
				bedai->sample_rate))
				adm_copp_mfc_cfg(
					get_port_id(bedai->port_id), copp_idx,
					bedai->sample_rate);
			...
			if ((fdai->perf_mode == LEGACY_PCM_MODE) &&
				(fdai->passthr_mode == LEGACY_PCM))
				msm_pcm_routing_cfg_pp(get_port_id(bedai->port_id), copp_idx,
						       topology, channels);
		}
	}
